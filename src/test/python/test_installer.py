...
from src.main.python.installer.app_installer import ApplicationInstaller
from src.main.python.model.application import Application
...
class TestApplicationInstaller(unittest.TestCase):
    @patch('src.main.python.repository.app_repo.AppRepository')
    @patch('src.main.python.installer.app_downloader.ApplicationDownloader')
    @patch('src.main.python.installer.factory.installer_factory.InstallerFactory')
    ...
    @patch('src.main.python.installer.factory.runner.CommandRunner.run')
    def test_installer_installs_application(self, run):
        ...
        # Then
        self.mock_repo.load_app.assert_called_with('1')
        self.mock_downloader.download.assert_called_once_with('http://somesite/foo.exe', 'foo', 'exe')
